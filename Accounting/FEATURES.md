...
| FEAT-AC11 | OAuth ログイン補助(会計は loopback リスナで `code` 自動受信→token保存・`ac mf login`/`refresh`/`token`・access/refresh を gitignore の `var/moneyforward/` に保存し失効時 refresh 自動更新。経費は手動) | **実装済・実 API e2e 確認済(2026-06-14。login/再利用/refresh)** | [core/oauth.py](core/oauth.py), [core/token_store.py](core/token_store.py), [scripts/oauth_listener.py](scripts/oauth_listener.py), [scripts/cli.py](scripts/cli.py) | REQ-EX01, REQ-EX06 |
