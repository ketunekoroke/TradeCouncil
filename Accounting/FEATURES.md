...
| FEAT-AC06 | MoneyForward API 疎通スパイク(設定を使い OAuth → offices。手動実行・実 creds 必要。token リクエスト組立・offices 件数抽出は純粋関数として単体テスト済) | **会計の実 API 疎通 確認済(2026-06-14:OAuth→`/v2/tenant`)**。経費は未疎通 | [scripts/spike_moneyforward.py](scripts/spike_moneyforward.py) | REQ-EX01, REQ-EX06 |
