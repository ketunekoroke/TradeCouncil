...
| FEAT-AC06 | MoneyForward API 疎通スパイク(設定を使い OAuth → offices。手動実行・実 creds 必要。token リクエスト組立・offices 件数抽出は純粋関数として単体テスト済) | 雛形(設定連携・純粋ロジック検証済) | [scripts/spike_moneyforward.py](scripts/spike_moneyforward.py) | REQ-EX01, REQ-EX06 |
