...
| TC-AC21 | 過去分の取込・補正: 純粋(`MFCurrent.from_tx` 正規化・`mf_file`→has_file・差分=名前正規化/Decimal/fx flagged無・ID未解決は変更外・`build_update_body` 変更キーのみ/空→`{}`・`applied` で冪等)+ 証憑バイナリDL注入/404 伝播 + パイプライン(期外除外・証憑なしフラグ・`past_<id>` 台帳/snapshot・冪等再取込・dry-run/`--confirm` で **変更キーのみ PUT・receipt_input なし**・`revised`+`revised_at`・差分ゼロ skip・証憑なしは skip) | `tests/test_revise.py`(純粋)`tests/test_mf_expense_api.py`(DL注入)`tests/test_expense_cli.py`(list/download/update 注入・無ネットワーク) | P1 | REQ-EX01, REQ-EX02 |
