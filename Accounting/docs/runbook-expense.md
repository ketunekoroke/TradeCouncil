...
4. **登録前レビュー(ユーザーが内容を確認)**: `expense review --push` — 未登録明細を一覧化
   (日付・取引先・金額・費目・税区分・確信度・要確認フラグ + **各証憑の SharePoint リンク**)した
   Markdown を生成し SharePoint ドキュメント/Expense/ へ push。**ユーザーが一覧を確認し、迷う明細は
   「証憑を開く」リンクをクリックして中身を見て判断**する。(`register` ドライランでも送信予定は確認可)
5. **ユーザー承認後**: `expense register --cloud-dedup --confirm` — MF へ登録(証憑添付=電帳法)+ Teams 通知。
   `--cloud-dedup` はクラウド実体と (取引日+金額) で突合し、一致を「要個別確認」で skip・報告(→ 重複判定の節)。
...
### 重複判定(重要)
- **ファイル名は識別子にしない**(`invoice (N).pdf` は Windows のDL重複採番。同名でDLするサービスは複数。
  同名で中身が別の再DLもある)。**content-hash + 請求書番号/注文番号 + (取引日+金額)** で判定する。
- **クラウド実データと突合**: `register --cloud-dedup`(`cloud_dupe_index`)でクラウド経費の (取引日+金額) と
  突合。ローカル台帳は **ステイル**(同名ファイルに前バッチの古いサイドカーが残る)で誤判定し得るのでクラウドが正。
- **(取引日+金額)一致は「重複」ではなく「個別確認のトリガー」**。一致したら請求書番号・品目(書名等)まで見て、
  **別物なら登録する**(例: Amazon の別書籍は同日同額でも書名が違う)。`--cloud-dedup` は一致を「要個別確認」で
  skip・報告する(自動登録も自動skip確定もしない)。別物と分かったら支払先に識別子(書名・入庫時刻等)を付けて
  別エントリで登録。
- 同名再DLで中身が変わった場合は **古いサイドカーを削除 → 現ファイルから再抽出**(ステイル・サイドカー是正)。
