...
経費は Redirect URI が **HTTPS 限定**で loopback リスナでは受けられないため手動運用:

1. アプリ登録の Redirect URI に上表の値を入れる(config の `redirect_uri` と完全一致させる)。
2. **認可 URL を生成しブラウザを開く**(`mf login --no-listen` か `mf authorize`。`.env` の client_id・config の
   redirect_uri/scopes から自動生成):
   ```powershell
   ..\.venv\Scripts\python.exe -m scripts.cli mf login --product expense --no-listen
   ```
...
3. 遷移先 URL の **`code=` をアドレスバーからコピー** → `.env` の
   `MONEYFORWARD_EXPENSE_AUTH_CODE`(会計を手動でやる場合は `MONEYFORWARD_ACCOUNTING_AUTH_CODE`)に貼る
   (表示された `state` が戻り先 URL の `state` と一致することも確認)。
4. `python -m scripts.spike_moneyforward --product <accounting|expense>` でトークン交換・保存。

> 経費で自前の `https://localhost` を使うには自己署名証明書が要る。提供の `oauth2-redirect.html` か、
> `ngrok` 等のトンネル(一時 HTTPS → localhost)が手軽。
...
1. **会計(推奨)**: `python -m scripts.cli mf login --product accounting`(B' 節 — loopback で `code` 自動受信 →
   token 交換 → `var/moneyforward/` に保存)。**経費**: `mf login --product expense`(自動で手動フローに切替 →
   表示 URL で許可 → `code` を `MONEYFORWARD_EXPENSE_AUTH_CODE` に設定 → 下記 spike)。
