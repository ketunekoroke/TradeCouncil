...
入出力はすべて単一の **`workspace/`** 配下に集約される(ADR-0009。local/・sharepoint/ の
二重ツリーは廃止)。SharePoint 連携のオン/オフ(`sharepoint.config.json` の `enabled`)は
**同期するかどうか**だけを変え、作業場所は常に `workspace/`:

| シナリオ | 出力先 |
|---|---|
| 意思決定会議 | `workspace/council/`(議事録・git 追跡)+ `config/policies/`(決裁済みポリシー) |
