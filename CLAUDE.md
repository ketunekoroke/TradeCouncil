...
`docs/03_運営規程・第0回アジェンダ.md`。設計書からの逸脱は `docs/adr/` に記録されている。
...
5. **テスト必須**: `core/risk/` と `core/execution/` の変更はテスト先行。
...
仕様・機能を変更したら: `docs/`(一次資料 01〜04。**直接改訂してよい**。大きな判断の経緯は
ADR に記録)→ `DOCS.md` → `REQUIREMENTS.md` → `FEATURES.md` → `TESTCASES.md` を併せて
更新する。`core/db/models.py` の変更時は `docs/04_データベース設計書.md` を必ず併せて更新する。
詳細は `DEVELOPMENT.md`。
