...
> **このドキュメントの位置づけ**
> - 正式仕様の**一次資料は `docs/01〜06`**(要件定義書・基本設計書・運営規程・DB設計書・
>   開発フロー/実行環境方針・戦略開発ガイド)。本書は「実装された現在の姿」を一望するための
>   ガイドであり、docs/ と乖離させない
...
| 層 | 内容 | 実装状態 |
|---|---|---|
| L1 実行層(常駐・決定的) | BOT・リスクガード・執行・DB・通知(Teams/Discord)・監視 | **Phase 0 実装済**(paper のみ) |
