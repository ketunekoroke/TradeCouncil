...
| ID | 機能 | 状態 | 実装/定義 | 関連要件 |
|---|---|---|---|---|
| FEAT-30 | Teams/Discord 通知(backend 切替。Teams は Power Automate Workflows へ Adaptive Card、severity 色 + FactSet。マルチチャネルルーティング: 明示 channel > routing > default のフォールバック連鎖 — ADR-0003。未設定時ログfallback)。env 名は**プロジェクト別プレフィックス** `TEAMS_<env_prefix>_WORKFLOW_URL[_<CH>]`(TradeCouncil=TC・無印は後方互換 — ADR-0011) | 実装済 | [core/notify/notifier.py](core/notify/notifier.py) | REQ-O01 |
