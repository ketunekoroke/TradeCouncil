# テストケース一覧(Test Cases)— TradeCouncil(自動売買)

TradeCouncil の検証用テストケース集。**重要度ランク(P0〜P3)** で分類。関連:
[REQUIREMENTS.md](REQUIREMENTS.md) / [FEATURES.md](FEATURES.md)。
シナリオ・人格のテストは [../Magi/TESTCASES.md](../Magi/TESTCASES.md)、ブリッジ・SharePoint の
テストは [../shared/TESTCASES.md](../shared/TESTCASES.md)。
...
| **P1** | コア。主要なハッピーパス | core/・scenarios/ の変更時 | 一部 手動 |
| **P2** | 拡張。外部API・長時間 | リリース前・フル回帰 | API課金 / 長時間 |
| **P3** | エッジ。例外・環境依存 | 環境変更時 | 再現条件が特殊 |

> テストは TradeCouncil/ 内で `..\.venv\Scripts\python.exe -m scripts.cli test`(または `tc test`)。
...
| ID | タイトル | 実行方法 | 関連 |
|---|---|---|---|
| TC-001 | 全 pytest スイート(196件)が緑 | `python -m scripts.cli test` | 全FEAT |
...
| TC-013 | 全スクリプト構文チェック | `python -m py_compile scripts/*.py scripts/hooks/*.py ../shared/*.py ../shared/hooks/*.py` | — |
| TC-014 | Claude フック単体: live/resume ブロック、generated/policies/var 保護、秘密検出 | stdin JSON を hooks に流して exit code 確認(FEAT-40〜41) | REQ-S01〜S03 |
| TC-015 | pre-commit: 決裁レコードなしポリシー / .env / 秘密 を拒否(shared/hooks) | ダミーをステージして `python ../shared/hooks/pre_commit.py` | REQ-S01, S03 |
...
| TC-022 | CLI 出力の cp932 耐性: 非対応グリフ(✓✗)は「?」置換で落ちない・encoding 不変・`status` が cp932 厳格ストリームで完走 | `pytest tests/scripts/test_cli_encoding.py` | FEAT-32 |
...
| TC-024 | BybitAdapter(フェイク ccxt 注入): mainnet 拒否・APIキー欠落の即エラー・注文変換と orderLinkId・約定解決・取引所エラー→rejected 記録・残高/建玉導出・実スプレッド | `pytest tests/exchange/test_bybit_adapter.py` | REQ-M02 |
| TC-025 | BybitFeed + JPY 換算: 確定バーのみ返す・data_age 実測・FxConfig fail-closed(未設定/未対応通貨/レート<1 拒否)・notional_jpy 換算・runner の equity/exposure/est_max_loss 換算 | `pytest tests/exchange/test_bybit_feed.py tests/risk/test_fx.py tests/runner` | REQ-M06, REQ-D01 |
...
| TC-206 | Bybit testnet 実接続検証: testnet キーで `dummy_rw_bybit` を数バー稼働 → orders/fills と Bybit 注文履歴の一致・実手数料・レイテンシ・再起動 reconcile | docs/setup/bybit-testnet-setup.md §6。⚠️ 制限対象国(米国等)の IP は 403 — VPN 出口国に注意 | FEAT-28, FEAT-29 |
