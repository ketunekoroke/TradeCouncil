...
- **`Magi` ⇎ `TradeCouncil` は相互非依存**。`Magi/` を削除しても売買は `tc test` 緑のまま
  (MAGI への実行時依存ゼロ)。`TradeCouncil/` を削除しても Magi の4シナリオは無傷
- `shared/` は両者の土台。削除すると外部 LLM/SharePoint 連携が縮退するが、各プロジェクト
  固有ロジックは生存する
