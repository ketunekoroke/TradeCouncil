...
|---|---|---|
| テナントID | **ディレクトリ (テナント) ID** | `SHAREPOINT_TENANT_ID` |
| クライアントID | **アプリケーション (クライアント) ID** | `SHAREPOINT_CLIENT_ID` |
...
| 値 | 後で使う環境変数 |
|---|---|
| クライアントシークレットの**値** | `SHAREPOINT_CLIENT_SECRET` |
...
SHAREPOINT_CLIENT_SECRET=<クライアント シークレットの値>
# 任意: ライブラリ名の上書き(全プロジェクト共通)
#SHAREPOINT_DRIVE=ドキュメント
# 注: 同期先フォルダ(root)はプロジェクトごとに */sharepoint.config.json で持つ(Magi/Workspace・
#     TradeCouncil/Workspace 等)。SHAREPOINT_ROOT は env で分けられないため通常は設定しない。
```
...
| drive 名の警告 `drive '…' が見つからない` | `drive` 名がライブラリ表示名と不一致(日本語テナントの既定は「ドキュメント」) | 既定ライブラリへ自動フォールバックするので動作はする。warn を消すには `SHAREPOINT_DRIVE=ドキュメント` |
| `SharePoint 無効(enabled=false)…` | `enabled` が `false` | `.env` の `SHAREPOINT_ENABLED=true`(env が config より優先) |
