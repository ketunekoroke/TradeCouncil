...
4. **地域ブロック(2026-06-12 実測)**: Bybit API は testnet・mainnet とも**日本の IP を
   403 で遮断**する(Bybit は金融庁未登録)。日本国内の開発機からは接続検証(TC-206)を
   実施できない。実装はフェイク注入の単体テストで検証済みであり、接続検証は Bybit が
   サービス提供する地域のネットワークでのみ行う。**規制回避を目的とした迂回
   (VPN 等)は行わない**。国内から検証可能な取引所への切替が必要になった場合は
   別 ADR で再決定する(アダプタは ccxt 共通 IF のため移植費用は小さい)
