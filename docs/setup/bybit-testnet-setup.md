...
> 🚫 **地域ブロック(2026-06-12 実測)**: Bybit API(testnet・mainnet とも)は
> **日本の IP からのアクセスを 403 で遮断**する(CloudFront の国別ブロック。
> Bybit は金融庁未登録)。日本国内の開発機からは公開データ取得を含め接続できない。
> 接続検証には Bybit がサービス提供する地域のネットワークが必要。
> **規制回避を目的とした迂回は行わない**こと(ADR-0008 既知の制約4)。
> 接続できる環境がない場合の代替は BACKLOG(取引所選定の再検討)を参照。
