# シナリオ・ブリッジ基盤 詳細テストケース集

シナリオ・ブリッジ基盤(MAGI 継承)の詳細テストケース集。**本ファイル内の TC 番号は
独立名前空間**であり、ルートの [TESTCASES.md](../../TESTCASES.md)(システム全体の管理表)とは
別体系。索引はルート TESTCASES.md から参照される。

- **出自**: `prototype/TESTCASES.md` を現行仕様に更新して移設(2026-06-13)。prototype/ は
  独立した参照資料(編集禁止)であり、本ファイルが現行の正本。
- **カレントディレクトリ**: すべての手順は**リポジトリルート**(`C:\git_repository\TradeCouncil`)で
  実行する(旧 prototype/ 基準から変更)。
- **Python 実行環境**: コマンド例は `python scripts/xxx.py` と表記するが、本リポジトリでは
  venv の Python(`.venv\Scripts\python.exe`)を使うこと(以降の例では `python` と略記)。
- **FEAT-xx / REQ-xx**: 出自である `prototype/FEATURES.md` / `prototype/REQUIREMENTS.md` の
  番号(トレーサビリティのため維持)。
- **入出力 root**: 旧仕様の `local/` / `sharepoint/` 二重ツリーは **ADR-0009 で単一の
  `workspace/` に統合済み**。`sharepoint.config.json` の `enabled` は「SharePoint と同期通信
  するか」のみを制御し、作業場所は常に `workspace/`。

**重要度ランク(P0〜P3)で分類**してあり、毎回すべてを流す必要はない。変更内容に応じて
下の「テスト選択ガイド」で対象を絞る。
...
| TC-005 | キー解決の優先順位(env→.env→settings) | P0 | ✕ | ◯ | FEAT-13, REQ-S03 |
...
| TC-012 | 入出力 root の Git 除外健全性(workspace/) | P0 | ✕ | ◯ | REQ-SC05, REQ-SP02 |
...
| TC-025 | ドキュメント整合(CLAUDE.md↔scenarios/↔docs/07) | P1 | ✕ | 手動 | REQ-B03 |
...
| TC-059 | sharepoint.py 構文 + root/status の整合 + sync 計画(オフライン) | P0 | ✕ | ◯ | FEAT-56, REQ-SP02, REQ-SP03 |
| TC-060 | enabled=false で sync/pull/push が no-op(安全) | P0 | ✕ | ◯ | FEAT-61, REQ-SP02 |
| TC-061 | 認証情報未設定エラーの整形(test) | P1 | ✕ | ◯ | FEAT-57, REQ-SP04, REQ-N03 |
| TC-062 | SharePoint 実 sync 往復(要テナント) | P3 | ◯ | △ | FEAT-58, FEAT-59, FEAT-60 |
...
> 注: TC-028 / TC-029 は出自の番号事情で**2件ずつ存在**する(P0 のフォールバック分岐 /
> P2 の Office 抽出系)。相互参照を壊さないため番号はそのまま維持している。
...
- **前提**: 環境変数 `OPENAI_API_KEY` 未設定、ルート `.env` に未記載(または placeholder)、
  かつ `.claude/settings.local.json` の env も placeholder。
- **手順**: `echo x | python scripts/ask_openai.py --system "t" --model gpt-4o`(キー無し状態)
- **期待**: `error: OPENAI_API_KEY が未設定です…` の1行で終了。スタックトレースなし。
- **自動テスト**: キー解決層は `tests/scripts/test_env_resolution.py` でモック検証済み
  (`python -m scripts.cli test` に含まれる)。

### TC-004 placeholder は未設定扱い
- **目的**: テンプレ値 `...REPLACE...` を誤って有効キーと見なさない(`.env` /
  settings.local.json のどちらに書かれていても)。
- **手順**: `.env` または settings.local.json の値が `...REPLACE...` の状態で TC-003 を実行。
- **期待**: 「未設定」として扱われ、次の解決ソースへフォールバック(全ソース未設定ならエラー。
  誤って API を叩かない)。空文字も同様に未設定扱い。
- **自動テスト**: `tests/scripts/test_env_resolution.py::test_get_setting_skips_placeholder_and_empty`。

### TC-005 キー解決の優先順位(env→.env→settings)
- **目的**: シークレットは **OS 環境変数 → ルートの `.env`(推奨・シークレット集約先)→
  `.claude/settings.local.json` の env** の3段で解決される(`bridge_common.get_setting` /
  `dotenv_env` が実装)。
- **手順**(自動テストが正本):
  ```bash
  python -m pytest tests/scripts/test_env_resolution.py -v
  ```
  手動で配線を見たい場合:
  ```bash
  ...
  ```
- **期待**: pytest 全緑。①OS 環境変数が最優先、②無ければ `.env`、③無ければ
  settings.local.json。placeholder・空文字はスキップして次ソースへ。複数名指定
  (例: `GEMINI_API_KEY, GOOGLE_API_KEY`)は先に見つかったものを返す。

### TC-006 settings JSON の妥当性
- **目的**: 設定ファイルとテンプレが壊れた JSON でない。
- **手順**: `python -c "import json,os; [json.load(open(p,encoding='utf-8')) for p in ['.claude/settings.local.json.example','sharepoint.config.json'] + (['.claude/settings.local.json'] if os.path.exists('.claude/settings.local.json') else [])]; print('ok')"`
- **期待**: `ok`(`.claude/settings.local.json` は存在する環境でのみ検査)。

### TC-007 Git 追跡/除外の健全性
- **目的**: 秘密ファイルが追跡されず、テンプレは追跡される(ルートの .gitignore 基準)。
- **手順**:
  ```bash
  git check-ignore .env .claude/settings.local.json        # 出る(=除外)
  git check-ignore .claude/settings.local.json.example     # 出ない(=追跡)
  # 入出力 root は単一の workspace/(ADR-0009)。具体ファイルパスで確認する:
  git check-ignore workspace/deliberations/x.md workspace/input/x.png "workspace/reviews/x-改訂版.docx"   # 出る(=除外)
  git check-ignore sharepoint.config.json                  # 出ない(=追跡。非機密設定)
  ```
- **期待**: `.env`・settings 実値・入出力ファイル(`workspace/` 配下)は除外、`.example` と
  `sharepoint.config.json` は追跡。議事録の追跡例外は TC-012 で検査。

### TC-008 人格 frontmatter の妥当性
- **目的**: 全人格 — MAGI 3人格(melchior / balthasar / casper)+ TradeCouncil ペルソナ5名
  (macro_analyst / momentum_trader / contrarian_value / quant_validator / risk_manager)—
  すべてに `backend`(claude|openai|gemini)と `model` がある。
- **手順**: 上記8ファイル(`.claude/agents/<name>.md`)の frontmatter を確認
  (code-reviewer / risk-auditor は開発用サブエージェントのため対象外)。
...
- **手順**:
  ```bash
  ls scenarios/   # council.md / deliberation.md / document-review.md / brainstorm.md / persona-test.md / README.md
  grep -o "scenarios/[a-z-]*\.md" CLAUDE.md | sort -u   # ルーター表が参照するファイル一覧
  ```
- **期待**: CLAUDE.md「シナリオ実行モード」のルーター表が参照する**5つ**の
  `scenarios/<name>.md`(council / deliberation / document-review / brainstorm / persona-test)が
  すべて実在し、`scenarios/README.md` の一覧とも一致する(リンク切れ・取りこぼしなし)。

### TC-012 入出力 root の Git 除外健全性
- **目的**: 入出力ファイル(元資料・成果物・個人情報を含みうる)が誤って追跡されない。
  単一 root `workspace/`(ADR-0009)の除外と、**会議議事録(.md)の追跡例外**(ADR-0005)の両立を検査。
- **手順**:
  ```bash
  git check-ignore "workspace/reviews/sample-改訂版.docx" workspace/input/x.png   # 出る(=除外)
  git check-ignore workspace/reviews/.gitkeep workspace/README.md                # 出ない(=追跡)
  git check-ignore workspace/council/2026-01-01-第N回会議.md                      # 出ない(=追跡例外)
  git check-ignore workspace/council/recording.mp4                               # 出る(=議事録例外は .md のみ)
  ```
- **期待**: `workspace/` 配下の成果物・入力は除外、`.gitkeep`・`README.md` は追跡、
  `workspace/council/**/*.md`(議事録=監査テキスト)のみ追跡例外。メディアは council 配下でも除外。
...
### TC-059 sharepoint.py 構文 + root/status の整合 + sync 計画(オフライン)
- **目的**: 同期ブリッジが壊れておらず、root 解決と sync 計画(plan_sync)が現行仕様
  (ADR-0009)どおりであること(通信なし)。
- **手順**:
  ```bash
  python -m py_compile scripts/sharepoint.py
  python scripts/sharepoint.py root      # 常に …/workspace で終わる(enabled の真偽によらず)
  python scripts/sharepoint.py status    # enabled / root / site / drive / folders を表示(通信なし)
  python -m pytest tests/scripts/test_sharepoint_sync.py -v   # plan_sync・root 解決の自動テスト
  ```
- **期待**: 構文 OK。`root` は **enabled の真偽によらず常に `workspace/`**(local/sharepoint の
  二重ツリーは廃止)。`status` が設定値(enabled / site / drive / folders)と一致して表示され、
  `enabled=false` のときは「sync/pull/push は通信しない」旨の注記が出る。pytest 全緑:
  `plan_sync` が **双方向・追加型・newer-wins・削除非伝播**(片側にしか無いファイルはコピー、
  delete アクションを生まない)、タイムスタンプ許容誤差内は skip、除外規則
  (`.gitkeep` / workspace 直下 README.md / `*.tmp`)を満たす。
  env `MAGI_SHAREPOINT_ENABLED` は config の `enabled` より優先(`true`/`1`/`yes`/`on`=オン。
  settings 経由でも同様に効く)。

### TC-060 enabled=false で sync/pull/push が no-op(安全)
- **目的**: SharePoint 無効時に同期コマンドを呼んでもローカル(workspace/)を壊さず安全に終了する。
- **手順**: `sharepoint.config.json` の `enabled:false`(既定)で
  `python scripts/sharepoint.py sync` / `python scripts/sharepoint.py pull` /
  `python scripts/sharepoint.py push`。
- **期待**: 「SharePoint 無効(enabled=false)。sync/pull/push は何もしません…
  workspace/ をそのまま使ってください」を stderr に出し、何もせず終了コード 0。
  ネットワークを叩かない。`workspace/` のファイルは変化しない。
...
- **目的**: 画像をネイティブに読む。
- **前提**: 任意の画像(例 `workspace/input/sample.png`)。
- **手順**: `echo "添付画像に見えるものを1文で。" | python scripts/ask_openai.py --system-file .claude/agents/melchior.md --model gpt-4o --file workspace/input/sample.png`
- **期待**: 画像の内容(文字・図形など)を具体的に描写する。

### TC-023 PDF(テキスト有)読取
- **目的**: テキストを持つ PDF をネイティブに読む。
- **前提**: 本文テキストを持つ PDF(スキャン画像のみでない)。
- **手順**: `echo "添付PDFのタイトルと結論を引用して。" | python scripts/ask_openai.py --system-file .claude/agents/melchior.md --model gpt-4o --file workspace/input/sample.pdf`
- **期待**: タイトル・本文を引用できる。

### TC-024 xlsx 抽出
- **目的**: Excel をローカル抽出して数値を扱える。
- **前提**: 表データを持つ xlsx。
- **手順**: `echo "添付の表から特定セルの値と傾向を答えて。" | python scripts/ask_openai.py --system-file .claude/agents/melchior.md --model gpt-4o --file workspace/input/sample.xlsx`
- **期待**: セル値を正しく答え、傾向を述べる。

### TC-025 ドキュメント整合(CLAUDE.md ↔ scenarios/ ↔ docs/07_シナリオ・人格基盤.md)
- **目的**: 仕様の乖離防止(本プロジェクトの禁止事項)。
- **手順**: backend/ファイル仕様・シナリオのラウンド/モード/成果物について、CLAUDE.md(共通作法)・
  `scenarios/<name>.md`(各シナリオ)・`docs/07_シナリオ・人格基盤.md`(一次資料。新設予定 —
  整備されるまでは DOCS.md の該当章を代替の照合先とする)を見比べる(手動レビュー)。
- **期待**: 共通作法は CLAUDE.md を唯一の出典としてシナリオに重複コピーがなく、各シナリオの
  モード表・ラウンド・成果物が一次資料と一致。出力先(`workspace/council/` /
  `workspace/deliberations/` / `workspace/reviews/` / `workspace/brainstorms/` /
  `workspace/persona-tests/`)が CLAUDE.md・各シナリオ・.gitignore で一致。
...
- **検証メモ**: 2026-06-05 に prototype 環境で上記を自動実行し PASS(変更反映+構造保持+原本不変)。

### TC-061 認証情報未設定エラーの整形(test)
- **目的**: SharePoint 有効でも認証情報が無いとき、トレースバックでなく整形メッセージで止まる。
- **前提**: `enabled:true` かつ `MAGI_SHAREPOINT_*` 未設定(.env / settings とも placeholder)。
...
- **目的**: Azure セットアップ手順書が存在し、DOCS から参照され、記載(env 名・必要権限・
  コマンド)がコード/設定と食い違わない(古いマニュアルで詰まらせない)。
- **手順**:
  ```bash
  test -f docs/setup/sharepoint-azure-app-setup.md          # 存在
  grep -q "docs/setup/sharepoint-azure-app-setup.md" DOCS.md   # 参照リンクあり
  # マニュアルが実装と同じ語彙を使っているか(env 名・権限・サブコマンド)
  M=docs/setup/sharepoint-azure-app-setup.md
  grep -q "MAGI_SHAREPOINT_TENANT_ID" "$M" && grep -q "MAGI_SHAREPOINT_CLIENT_SECRET" "$M"
  grep -q "MAGI_SHAREPOINT_ENABLED" "$M"   # 同期通信のオンオフを切替できる旨
  grep -q "Sites.ReadWrite.All" "$M"
  for c in status test sync info; do grep -q "sharepoint.py $c" "$M" || echo "missing: $c"; done
  ```
- **期待**: すべて真(`missing:` 行が出ない)。マニュアルの env 名は `bridge_common`/`sharepoint.py`、
  権限はマニュアル §3、サブコマンドは `sharepoint.py` の実装と一致する(主動線は `sync`。
  `pull`/`push` は選択的リカバリ用の補助)。
...
- **手順**:
  ```bash
  FID=$(python scripts/ask_openai.py upload workspace/input/sample.png)
...
- **手順**: リポジトリルートで `claude` を起動し、合議(deliberation)の Lite 議題を1件回す。
...
- **手順**: 任意モードで合議。
- **期待**: `workspace/deliberations/YYYYMMDD-HHMM-<議題>.md` が生成される。

### TC-042 資料レビュー一周(Standard、指摘+改訂版)(手動)
- **手順**: リポジトリルートで `claude` を起動して資料を渡し、document-review シナリオを Standard で1件回す。
  例: `workspace/input/` に短い docx/md を置き「この資料をレビューして改訂版も作って」。
- **期待**: Round 0,1,2,3,4,6 が進み、3レンズの指摘 → 裁定 → 改訂版生成まで到達。
  `workspace/reviews/` にレビューレポートと改訂版の両方が出る。

### TC-043 改訂版が元と同形式で出る(手動)
- **手順**: 元が docx の資料と、元が md の資料でそれぞれレビューを回す。
- **期待**: docx 入力 → `workspace/reviews/...-改訂版.docx`、md 入力 → `workspace/reviews/...-改訂版.md`(形式が保たれる)。
...
- **手順**: リポジトリルートで `claude` を起動し、軽いテーマを Standard で1件回す。
  例: 「エンジニアの学びが続く社内勉強会のネタ」(制約: 月1回・1時間・登壇持ち回り)。
- **期待**: Round 0,1,2,3,4,5,6,7,8 が進み、`workspace/brainstorms/` にアイデア集が出る。アイデア集に
...
- **手順**: リポジトリルートで `claude` を起動し、人格テストを Standard で1件実行(P1〜P4 を3人格に独立投下)。
  人格調整の検証なら Round 0 で「何を変えたか」を宣言して回す。
- **期待**: Round 0〜4 が進み、`workspace/persona-tests/` に比較レポートが出る。レポートに
...
- **手順**: 成果物形式を指定して合議。
- **期待**: 指定形式のファイルが `workspace/deliberations/` / `workspace/media-output/` に生成される。
...
- **手順**: リポジトリルートで `claude` を起動し、軽いテーマを Quick で1件回す。
- **期待**: Round 0,1,2,4,5,6,7,8 のみが進み(掛け合わせ発散・ループ・プレモーテムは無し)、
  `workspace/brainstorms/` にアイデア集(Markdown + Mermaid マップ)が出る。
...
### TC-G01 キー未設定エラー
- **前提**: `GEMINI_API_KEY` / `GOOGLE_API_KEY` 未設定(env・ルート `.env`・settings の3段とも
  未設定または placeholder)。
...
- **手順**: `echo "添付画像に見えるものを1文で。" | python scripts/ask_gemini.py --system-file .claude/agents/melchior.md --model gemini-2.5-flash --file workspace/input/sample.png`
- **期待**: 画像内容を具体的に描写する。

### TC-G04 PDF(テキスト有)読取
- **手順**: `echo "添付PDFのタイトルと結論を引用して。" | python scripts/ask_gemini.py --system-file .claude/agents/melchior.md --model gemini-2.5-flash --file workspace/input/sample.pdf`
- **期待**: タイトル・本文を引用できる。

### TC-G05 upload→file-id 使い回し
- **手順**:
  ```bash
  FID=$(python scripts/ask_gemini.py upload workspace/input/sample.png)   # files/xxxx
...
### TC-062 SharePoint 実 sync 往復(要テナント)
- **目的**: 実際の SharePoint と `workspace/` が `sync` で往復することの統合確認
  (環境依存・要 Azure アプリ + テナント)。
- **前提**: `enabled:true`、`MAGI_SHAREPOINT_*` 設定済み(推奨: ルート `.env`)、
  `Sites.ReadWrite.All` 同意済み(または `Sites.Selected` + 対象サイトへ `write` 付与)。
- **手順**:
  ```bash
  python scripts/sharepoint.py test                 # 認証 + site/drive 解決
  # workspace/reviews/ にテストファイルを作成してから:
  python scripts/sharepoint.py sync                 # 双方向同期(ローカル新規 → push される)
  python scripts/sharepoint.py sync                 # 2回目: 差分なし → 全件 skip
  # ローカル側のテストファイルを削除してから:
  python scripts/sharepoint.py sync                 # 追加型: 遠隔に残っている → pull で復活(削除は伝播しない)
  python scripts/sharepoint.py info workspace/reviews/<file>   # Web URL
  # 選択的リカバリの補助コマンド(任意):
  python scripts/sharepoint.py pull input           # 遠隔 input → workspace/input/ のみ
  python scripts/sharepoint.py push reviews         # workspace/reviews/ のみ → 遠隔
  ```
- **期待**: `test` 成功。1回目の `sync` でローカル新規が遠隔へ push、**2回目の `sync` は全件
  skip**(newer-wins 判定で差分なし)。ローカル削除後の `sync` では遠隔から pull され復活
  (**削除非伝播=追加型**)。`info` が当該アイテムの `webUrl` を返す。4MB 超のファイルでも
  アップロードセッションで分割成功(任意)。
- **検証メモ**: 2026-06-10 に実テナント(`kitune.sharepoint.com/sites/Magi`、`Sites.Selected`+`write`)で
  ...
  リモート/ローカル後始末。当時は pull/push ベース)。既定ライブラリ名が `ドキュメント` のため
  `drive: Documents` は既定へフォールバックして動作(警告のみ。`MAGI_SHAREPOINT_DRIVE` で抑止可)。
  sync 計画ロジック自体はオフラインの自動テスト(→ TC-059)で常時検証される。
...
- **期待**: 手順書 [docs/setup/sharepoint-azure-app-setup.md](../setup/sharepoint-azure-app-setup.md) の
...
- 機能を追加したら、対応する **テストケースを本ファイルに追加**し、適切なランクを付ける
  (システム全体の管理表=ルート TESTCASES.md の索引も併せて更新する)。
...
- API 課金が気になる場合は、`gpt-4o-mini` など安価モデルで P1/P2 を回しても挙動確認は可能
  (人格の質は落ちるが配線の検証には十分)。
- pytest 化済みの範囲(キー解決=`tests/scripts/test_env_resolution.py`、sync 計画=
  `tests/scripts/test_sharepoint_sync.py`)は `python -m scripts.cli test` で常時回る。
  手順を変えたら自動テストも併せて更新する。
