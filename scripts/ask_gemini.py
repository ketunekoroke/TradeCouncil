...
環境変数(無ければ .claude/settings.local.json の env を読む):
  ...
  MAGI_GEMINI_FALLBACK_MODEL  任意。primary が過負荷/不在(429/5xx/404)のとき切り替える代替モデル(--fallback-model でも可)。
...
def cmd_run(argv):
    ...
    p.add_argument(
        ...
        "(env MAGI_GEMINI_FALLBACK_MODEL でも可)",
    )
    a = p.parse_args(argv)
    ...
    fallback = a.fallback_model or bc.get_setting("MAGI_GEMINI_FALLBACK_MODEL")
