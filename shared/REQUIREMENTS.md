...
| REQ-LB05 | openai/gemini 人格の拒否/空応答時は自動で再試行する(既定1回、`BRIDGE_GEN_MAX_RETRIES`)。続く場合はファシリテーターが人格定義を変えず指示を言い換えて再実行 | 推奨 | 実装済 |
...
| REQ-SP04 | 認証はアプリ(クライアントシークレット)。`SHAREPOINT_*`(旧 `MAGI_SHAREPOINT_*` 後方互換)を .env で解決しコミットしない | 任意 | 実装済 |
| REQ-SP13 | **接続をプロジェクトごとに切り替えられる**: 識別子(site_url/tenant_id/client_id)は各 `*/sharepoint.config.json` に持てる(非秘密)。client_secret は秘密なので env。各フィールドの解決順は プロジェクト別 env(`SHAREPOINT_<env_prefix>_*`)→ config → 共有 env。プロジェクト別アプリ登録/サイトに対応(ADR-0011) | 任意 | 実装済 |
...
| REQ-NF05 | 一過性の HTTP エラー(429 / 5xx / 接続タイムアウト)は指数バックオフ + ジッタで自動リトライする(`Retry-After` 尊重、回数・タイムアウトは `BRIDGE_HTTP_*` で調整可) | 推奨 | 実装済 |
...
| REQ-SH04 | 環境変数は共有層の設定のみに使い、**ドメイン別プレフィックス**で名付ける(`SHAREPOINT_*`/`BRIDGE_*`/`OFFICE_*`・プロバイダ標準キー)。プロジェクト名を env に入れない。プロジェクト別設定は config で分ける。旧 `MAGI_*` は `bc.setting()` が後方互換で読む非推奨エイリアス(ADR-0011) | 必須 | 実装済 |
